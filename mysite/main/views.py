    ...
    def create_personal_cc_report(month, year):
        ...
        for user in get_uniq_users:
            ...
            deposit_count = CRMCheck.objects \
                .filter(upload_date_short__icontains=filter_date, user_name=user_name, first_deposit_date=None) \
                .filter(~Q(call_result='есть депозит') & ~Q(call_result='чужой номер') & ~Q(call_result='номер не РБ')
                        & ~Q(call_result='нет ответа')) \
                .count()
